...
class PerformanceMetrics:
    ...
    def calculate_win_ratio(self):
        ...
        wins = self.trades[self.trades['signal'] > 0]['balance'].diff().dropna()
        return (wins > 0).sum() / len(wins) * 100

    def calculate_risk_of_ruin(self):
        """Estimate the Risk of Ruin."""
        total_trades = len(self.trades)
        losing_trades = self.trades[self.trades['balance'].diff() < 0]
        p_loss = len(losing_trades) / total_trades
        risk_of_ruin = (p_loss / (1 - p_loss)) ** total_trades
